...
var trait = function (req, res, query) {
    ...
    for (var idx = 0; idx < 5; idx++) {
        if (partie.batiment[idx] == query.chantier) {

            idx_chantier = partie.joueur[i].chantier_cours.length;

            if (idx_chantier === undefined) {
                idx_chantier = 0;
            }

            partie.joueur[i].chantier_cours[idx_chantier] = {};
            partie.joueur[i].chantier_cours[idx_chantier].id = Number(query.chantier);
            partie.joueur[i].chantier_cours[idx_chantier].ouvrier = [];

            partie.batiment.splice(idx, 1);
